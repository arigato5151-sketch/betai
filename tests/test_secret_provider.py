...
def test_vault_requires_sportmonks_token_when_provider_is_enabled(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    configure_vault_environment(monkeypatch)
    monkeypatch.setenv("SPORTMONKS_ENABLED", "true")
    payload = {key: "configured" for key in REQUIRED_VAULT_SECRET_KEYS}

    with pytest.raises(RuntimeError, match="SPORTMONKS_API_TOKEN"):
        load_external_secrets(
            Path("missing.env"),
            client_factory=Mock(return_value=vault_client(payload)),
        )

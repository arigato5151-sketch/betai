...
class MLModelPipeline:
    """Legacy single-model pipeline. Deprecated in favor of Tier1Model/Tier2Model.

    .. deprecated::
        Use ``app.prediction.ml.ml_pipeline.Tier1Model`` and ``Tier2Model``
        with ``MultiTierDatasetBuilder`` for new training workflows.
    """
    ...
    def __init__(self):
        ...
        _logger.warning(
            "MLModelPipeline is deprecated; use Tier1Model/Tier2Model instead."
        )
    ...
    def _save_active_model(
        ...
        # A unique temporary name keeps concurrent trainers (Celery beat +
        # manual retrain) from clobbering each other's half-written files.
        temporary_path = active_path.with_name(
            f"{active_path.name}.tmp.{uuid.uuid4().hex}"
        )
